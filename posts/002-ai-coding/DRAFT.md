...
这套流程还有几个明显可以优化的地方，留作后续迭代：

- **数据采集自动化**：目前 105 篇参考文章是手动收集整理后上传的，每季度更新时重复工作量较大。下一步可以考虑用爬虫或 RSS 订阅自动抓取国家统计局、Wind 等渠道的数据和文章，直接对接 NotebookLM 的 API，减少人工搬运。

- **多格式导出**：目前只有 Web 版，部分场景需要发 PDF 或 PPT。可以在 CI/CD 流程里加一步 Puppeteer 截图转 PDF，或者复用现有的 `pptx` skill 把内容同步输出一份演示文稿。

- **定制 Skill**：目前用的统计分析 Skill 是通用版本，对宏观经济场景有一定适配但不够精准，比如季节调整、基数效应修正等宏观特有的处理方式还需要手动在提示词里补充说明。长期来看值得为这个细分场景单独维护一个领域专用 Skill。
